...
function ActivityTimeline({ entityType, entityId, emptyKey, limit, filter }) {
  const data = useCtxAT(LifeDataContext);
  const { locale, t } = useCtxAT(LifeLocaleContext);
  if (!data) return null;
  const log = data.state.activityLog || [];
  let rows = LifeActivity.entriesFor(log, entityType, entityId);
...
function describeAction(entry, t, locale) {
  ...
  const lookup = LifeATLLabels && LifeATLLabels[locale === 'uk' ? 'uk' : 'ru'];
...
export { LifeATLLabels, ActivityTimeline };
