...
import { LifeMedStatusLabels } from '../../data/medications.js';
import { LifeMedMath } from '../../lib/medMath.js';
import { MedConfigDrawer } from './MedConfigDrawer.jsx';
import { PharmNotes } from './PharmNotes.jsx';
...
function MedDetailPage({ medId, onBack }) {
  const data = useCtxMD(LifeDataContext);
  const { t, locale } = useCtxMD(LifeLocaleContext);
  const I = LIcons;
  ...
  const statusLabels = LifeMedStatusLabels || {};
...
function DosesTab({ medId, med }) {
  ...
  return (
    <div className="mdp-doses">
      ...
      <ActivityTimeline entityType="med_dose" entityId={medId} limit={100} />
      <ActivityTimeline entityType="med_config" entityId={medId} limit={50} />
      <ActivityTimeline entityType="mode_style" entityId={medId} limit={20} />
    </div>
  );
}
