...
import { GlobalJournal } from './medications/GlobalJournal.jsx';
import { MedCard } from './medications/MedCard.jsx';
import { MedConfigDrawer } from './medications/MedConfigDrawer.jsx';
import { MedDetailPage } from './medications/MedDetailPage.jsx';
import { RefillModal } from './medications/RefillModal.jsx';
import { TakeDoseModal } from './medications/TakeDoseModal.jsx';
...
function MedicationsPage() {
  const data   = useCtxMP(LifeDataContext);
  const { t, locale } = useCtxMP(LifeLocaleContext);
  const I = LIcons;
  ...
  if (subId) {
    if (MedDetailPage) {
      return <MedDetailPage medId={subId} onBack={closeDetail} />;
    }
    return (
  ...
  return (
    <div className="page meds-page">
      ...
      {takeMed && <TakeDoseModal med={takeMed} onClose={() => setTakeMed(null)} />}
      {refillMed && <RefillModal med={refillMed} onClose={() => setRefMed(null)} />}
      {configMed && MedConfigDrawer && (
        <MedConfigDrawer med={configMed} onClose={() => setCfgMed(null)} />
      )}
      {configMed && !MedConfigDrawer && (
